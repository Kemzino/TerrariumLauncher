...
#### Канали й репозиторії збірок

| Збірка | Стабільний канал (для всіх)                 | Тестовий канал (адміни й тестери)                   |
| ------ | ------------------------------------------- | --------------------------------------------------- |
| Клієнт | `Kemzino/TerrariumCreate` — публічний       | `Kemzino/TerrariumCreateTest` — **приватний**       |
| Сервер | `Kemzino/TerrariumCreateServer` — приватний | `Kemzino/TerrariumCreateServerTest` — **приватний** |

- Обидва канали читають `releases/latest` свого репозиторію; у тест-репо релізи звичайні (не pre-release), інакше `latest` їх не бачить.
- Бета не публічна: тестовий репо бачить лише той, кого додано collaborator-ом (Read). Тестер робить
  fine-grained PAT з правом _Contents: read_ на тест-репо і вставляє його в налаштуваннях лаунчера — роль `tester`.
- «Опублікувати в тест» = реліз у тест-репо. «Поширити для всіх» = копія релізу (тег, назва, опис, `.mrpack`)
  у стабільний репо; якщо такий тег там уже є — тест вважається поширеним.
- Константи репо: `packages/app-lib/src/api/terrarium.rs` (`CLIENT_REPO`, `CLIENT_TEST_REPO`, …).
